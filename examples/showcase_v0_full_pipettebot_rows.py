...
Environment variables (all optional — modes chosen from env presence):
    ...
    PIPETTE_PROFILE      Path to a TOML experiment profile that specifies
                         ...
                         Profile length sets the cycle count (overrides
                         NUM_CYCLES). B2 PI_VOLUM is re-sent per-cycle
                         only when the volume changes from the prior cycle.
...
def _format_volumes(volumes_ul: tuple[float, ...]) -> str:
    """Describe a volume schedule — short form for constant, list for per-cycle."""
...
def _run(
    ...
    _phase_comment(
        gcode_out,
        _gcode_header(num_cycles, _format_volumes(volumes_ul))
    ...
    _phase_comment(
        gcode_out,
        "\n; ===== phase 2: per-channel volume — set per-cycle in phase 3 loop =====\n"
        "; B2 PI_VOLUM is re-sent only when the cycle's volume differs from the prior.\n",
    )
    ...
    _phase_comment(
        ...
        f"; per-channel volumes: {_format_volumes(volumes_ul)}\n"
        f"; total dispensed: {total_ul:.0f} uL across {num_cycles * 8} wells\n",
    )
    ...
    for n in range(1, num_cycles + 1):
        v = volumes_ul[n - 1]
        if pipette is not None and v != prev_volume:
...
def main() -> int:
    ...
    print(f"[host] {banner}")
    print(
        f"[host] row tour: {num_cycles} cycles × 8 channels = "
        f"{sum(volumes_ul) * 8:.0f} uL total"
    )
