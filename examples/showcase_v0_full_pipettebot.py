...
# (standard SBS column pitch). 11-cycle ladder: 190, 181, 172, …, 100.
SBS_REF_X = 25.0 + DECK_OFFSET_X  # 50.0 Marlin
SBS_COL1_Y = 190.0 + DECK_OFFSET_Y  # 190.0 Marlin (cycle 1)
SBS_COL_PITCH = -9.0  # -9 mm per cycle (standard SBS pitch)
...
def _format_volumes(volumes_ul: tuple[float, ...]) -> str:
    ...
    return f"per-column ({len(volumes_ul)} values: {list(volumes_ul)})"


def _gcode_header(volumes_ul: tuple[float, ...]) -> str:
    return (
        "; showcase_v0_full_pipettebot.gcode\n"
        "; generated by examples/showcase_v0_full_pipettebot.py\n"
        f"; full plate fill: {len(volumes_ul)} SBS cols x 8 channels, REAL dPette\n"
        f"; per-channel volume: {_format_volumes(volumes_ul)}\n"
...
def _run(
    ...
    """Full tour: bootstrap → tip pickup → N × (aspirate + dispense) → park.

    `volumes_ul[i]` is column-(i+1)'s per-channel volume; N = len(volumes_ul).
    B2 PI_VOLUM is re-sent only when the column's volume differs from
    the previous one — constant-volume tours pay exactly one B2 packet.
    ...
    _phase_comment(
        gcode_out,
        "\n; ===== phase 1: bootstrap (G28 X Y Z, no Z lift) =====\n"
        "; PRECONDITION: tips MUST BE REMOVED before G28 fires.\n"
        "; G28 X Y Z (explicit axes) — plain G28 skips Z on AI3M Marlin.\n"
        "; Phase 2's step 1 (Z → TIP_PICKUP_PRE_Z) handles the post-G28\n"
        "; Z ascent directly, so no separate `G1 Z=TRAVEL_Z` here.\n",
    )
    ...
    _phase_comment(
        ...
        f"; volumes per column: {_format_volumes(volumes_ul)}\n"
        f"; total dispensed: {total_ul:.0f} uL across {num_columns * 8} wells\n",
    )
    ...
    for col in range(1, num_columns + 1):
        ...
        v = volumes_ul[col - 1]
        print(f"\n[host] ====== COLUMN {col}/{num_columns} (volume={v:.1f} uL) ======")
        _phase_comment(
            gcode_out,
            f"\n; ====== COLUMN {col}/{num_columns} (volume={v:.1f} uL) ======\n",
        )
        if v != prev_volume:
            pipette.set_volume(v)  # B2 PI_VOLUM
            prev_volume = v
            _phase_comment(gcode_out, f"; >>> dpette set_volume({v:.1f} uL)\n")
