...
- `examples/showcase_v0_full_plate.py` — full 96-well plate fill tour. Phases: bootstrap (`G28 X Y` + Z raise — **no Z homing** since `Z=0` is calibrated to tip-on-deck and would drag mounted tips) → one-time tip pickup from back-right box → 12× (reservoir aspirate → SBS column dispense, back-to-front) → fast `G1` park at home corner with `PARK_Z = 1.5 × tip length` for forgotten-tip clearance. Encodes deck-layout constants directly. Aspirate and dispense are each a single descent (no plunger-stroke simulation). `XY_FEED = 200` mm/s. **Precondition: Z must be referenced before the tour runs** — run `home_G28_fast.py` with no tips loaded once per power cycle. Marlin's refusal of `G1 Z` on un-homed Z is the fail-safe.
- `examples/home_G28_fast.py` — standalone "home only" script that lands at Marlin default home `(X=0, Y=0, Z=0)` in roughly half the time of full `G28`. Four phases: `M203 Z40` / `M201` bump, defensive Z lift to 80 mm, `G28 X Y` (skip the slow Z leadscrew home), then `G1 Z0` at 40 mm/s. Trusts the tracked Z position instead of re-referencing the Z endstop — run full `G28` manually if Z drift is suspected. Total time reported.
