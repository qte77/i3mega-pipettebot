...
- `examples/*.py` bootstraps converge on a unified **liquid-handling motion profile** — `M203 X500 Y500 Z20` (peak feedrate caps, Z at leadscrew limit), `M201 X1200 Y1500 Z80` (per-axis max accel; Z softened from the AI3M default ~200 to protect the leadscrew and the cantilevered pipette), `M204 P1200 R1200 T1200` (default print/retract/travel accel), `M205 X3 Y5 Z0.2 E0` (classic jerk — tight X for tip-pendulum + droplet-shed control; tight Z for clean meniscus contact). Applied across all six gantry-using examples (`home_G28_fast`, `showcase_v0_pipette_sim`, `showcase_v0_full_plate`, `showcase_v0_full_pipettebot`, `showcase_v0_full_pipettebot_rows`, `showcase_v0_tip_pickup_release`) so chaining scripts leaves a known motion state. `home_G28_fast` previously bumped X/Y caps for "snappier" post-home motion (M203 X1000 Y1000, M201 X2000 Y2000); now aligned with the rest. Per-file "Side effect" docstrings and inline G-code tee comments reworded from "snappy strokes" to the liquid-handling rationale. `docs/deck-layout.md` bootstrap snippet updated to match.
