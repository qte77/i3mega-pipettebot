...
- `tools/smartto_probe.py` — read-only bring-up diagnostic for Smartto-firmware printers (Geeetech A30, GTM32 mini s board). Three phases at the working baud: identity (`M115`/`M114`), endstop state (`M119`; optional second poll after operator engages Z sensor), and capability probe of six non-motion commands (`M503`/`M400`/`M203`/`M204`/`M205`/`M501`) — each response classified SUPPORTED/UNSUPPORTED/PARTIAL/SILENT in a final summary table. Banned motion commands (`G0`–`G3`/`G28`/`G29`/`G30`/`G92`/`M84`/`M18`/`M500`) hard-refused; setters that *do* land are followed by `M501` so any RAM-only changes revert before exit. Auto-discovers baud via a small sweep (`115200,250000,57600,9600`).
- `tools/smartto_repl.py` — interactive G-code REPL for the Geeetech A30, companion to `tools/marlin_repl.py`. Reads `SMARTTO_PORT`; default baud 115200 (confirmed live on A30 fw v1.xx.58). Independent of `pipettebot.gantry` by design (115200 needs no `BOTHER` ioctl). Built-in `?` cheat-sheet covers Smartto's verified-working surface (`M115`/`M114`/`M119`/`G28 X Y`/`G92 Z0`/`M203`/`M204`/`M205`/`M220`/`M17`/`M18`) plus untested commands worth probing. Safety notes call out the known `G28 Z` dive on head-less A30 builds — operator must use `G28 X Y` plus manual `G92 Z0` instead.
- `docs/research/gantry-firmware-alternatives.md` — append-only research log evaluating gantry hardware + firmware targets against the Anycubic i3 Mega + stock Marlin baseline. First entry (2026-05-23) is the Geeetech A30: stock Smartto firmware verdict TRACK with adapter (live capability probe confirmed `M400`/`M203`/`M204`/`M205`/`M501` all supported on v1.37.58, contradicting the older Geeetech-forum command list); TheThomasD/GeeetechA30T Marlin port verdict SKIP for v0 (blocked by AGENTS rule #4). Includes a six-step v0 operational runbook (`G28 X Y` + manual jog + `G92 Z0`; no `G28 Z`, which dives because Smartto's homing routine watches a probe pin not wired in the head-removed configuration). Architecture revision: the MarlinGantry/SmarttoGantry adapter split is unnecessary now that capability parity is confirmed — a single `GcodeGantry` with a small per-device config (baud + homing strategy) covers both.
...
- Experiment profiles: `pipettebot.experiment_profile.load_experiment_profile` (TOML) + `PIPETTE_PROFILE` env var on both dpette showcases; per-cycle B2 PI_VOLUM only re-sent on change. Sample profiles for calibration curves and reservoir gradients in `examples/experiment_profiles/`. Closes Phase 1 of #79 (#82).
...
- **Motion-profile module + `MOTION_PROFILE` env selector.** New `src/pipettebot/motion_profile.py` bundles three named profiles — SLOW (extra-gentle, Z accel 80, X/Y 300/400, jerk 1.5/2/0.15), MID (default; liquid-handling friendly, Z at the leadscrew-friendly floor of 200, X/Y 600/800, jerk 3/5/0.2), FAST (quicker tours, Z accel 400, X/Y 1000/1200, jerk 5/8/0.4). Selected at runtime via `MOTION_PROFILE=slow|mid|fast`; empty string or `off` opts out (Marlin keeps RAM/EEPROM motion settings). Feedrate caps (M203 X500 Y500 Z20) shared across profiles. Applied across all six gantry-using examples (`home_G28_fast`, `showcase_v0_pipette_sim`, `showcase_v0_tip_pickup_release`, `showcase_v0_full_plate`, `showcase_v0_full_pipettebot`, `showcase_v0_full_pipettebot_rows`); each script's bootstrap calls `select_profile(os.environ.get("MOTION_PROFILE"))` and logs the active profile to stdout + the tee'd `.gcode` comments. Replaces the original PR-#113 hardcoded inline M-codes. Design rationale + alternatives in [ADR 0003](docs/adr/0003-motion-profile-bundled-constants.md).
- **Per-dive Z feedrate slowdowns** in `examples/showcase_v0_full_pipettebot_rows.py` for damage + liquid-surface protection. Tip-box engagement and bar engagement descents at 5 mm/s (F300); reservoir + well dives at 10 mm/s (F600). Lifts keep Z_FEED=1200 (20 mm/s). Scope intentionally limited to the rows script for now; roll-out to the other showcase scripts is a follow-up PR after hardware validation.
- **Touchdown split** in the four dive helpers of `examples/showcase_v0_full_pipettebot_rows.py` (`_pickup`, `_aspirate`, `_dispense`, `_eject`). New `TOUCHDOWN_APPROACH_MM=10.0` constant; each descent is now two `G1`s — fast `Z_FEED` until 10 mm above the contact point, then the slow contact feedrate for the final approach. The aspirate diagonal carries Y to target on the fast leg so the last 10 mm is pure-Z slow. Reduces per-dive wall-clock by ~80 % while preserving the contact-point protection.
- **Release-bar Y re-measured** in `examples/showcase_v0_full_pipettebot_rows.py`: the hook is attached to the i3 Mega's fixed chassis frame at the front of the machine (not the moving bed), so the bed slides forward until the commanded Marlin Y reads `RELEASE_BAR_Y = 220` at engagement. Earlier `190` was found to miss the hook on the real rig. Constant + docstring per-cycle gantry-shape block + phase-3 park comment all reconciled.
- **`PIPETTE_PROFILE` support in `examples/showcase_v0_full_pipettebot_rows.py`.** Pipes the rows showcase through the shared `pipettebot.cli_profile.build_volumes()` so it accepts profiles on the same precedence rules as the other v0 showcases (profile > `PIPETTE_VOLUME_UL` > 100.0 default; profile length overrides `NUM_CYCLES`). B2 PI_VOLUM moved from the one-shot phase 2 into the per-cycle loop with a `prev_volume` sentinel — constant-volume tours still pay exactly one B2 packet, profile tours re-send only on volume change. Phase 2 ("set volume once") dropped; row tour is now phase 2, park is phase 3.
- **Showcase scripts deduplicated** — `examples/showcase_v0_full_pipettebot.py` and `examples/showcase_v0_full_dpette_cycles.py` drop their local `_resolve_profile` / `_build_volumes` / `DEFAULT_VOLUME_UL` and route through `pipettebot.cli_profile.build_volumes()`. Behaviour is identical (same precedence, same banner shape).
- **Pipetting-profile surface renamed for naming symmetry** with the new motion_profile module: `src/pipettebot/profiles.py` → `src/pipettebot/experiment_profile.py`; `examples/profiles/` → `examples/experiment_profiles/`; `tests/test_profiles.py` → `tests/test_experiment_profile.py`; `load_profile()` → `load_experiment_profile()`. `ExperimentProfile` dataclass unchanged. All path refs updated across README, AGENTS, CONTRIBUTING, and the two example scripts that consume the loader.
