...
- **Release-bar coordinates corrected** in `examples/showcase_v0_full_pipettebot_rows.py`: `RELEASE_BAR_X` 0 → 5 (clear of X-min endstop) and `RELEASE_BAR_Y` 220 → 190 (the bar's actual Y, per user measurement — was previously confused with bed Y-max).
