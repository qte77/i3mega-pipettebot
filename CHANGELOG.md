...
- `examples/showcase_v0_full_plate.py` — full 96-well plate fill tour. Phases: bootstrap (Z raise only — **no auto-`G28`**, since Z homing drives the calibrated tip-end into the deck) → one-time tip pickup from back-right box → 12× (reservoir aspirate → SBS column dispense, back-to-front) → fast `G1` park at home corner with `PARK_Z = 1.5 × tip length` for forgotten-tip clearance. Encodes deck-layout constants directly. Aspirate and dispense are each a single descent (no plunger-stroke simulation). `XY_FEED = 200` mm/s. **Precondition: printer must be homed before the tour runs** — Marlin's refusal of `G1` on un-homed axes is the fail-safe.
