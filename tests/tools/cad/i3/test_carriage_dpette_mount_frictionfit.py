"""Tests for tools/cad/i3/carriage_dpette_mount_frictionfit.py — TESTING variant.

Mirrors test_carriage_dpette_mount.py's structure for the friction-fit
testing design (see that module's docstring for why this is a separate,
self-contained iteration rather than a variant of the production bolted
mount). Asserts the AI3M payload budget (mount + pipette + tips < 300 g)
and geometric invariants. Marker `cad` — requires build123d
...
def _import_mount():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "carriage_dpette_mount_frictionfit",
            CAD_DIR / "i3" / "carriage_dpette_mount_frictionfit.py",
        )
...
TIPS_MASS_G = 3.0  # 8 x 300 uL polypropylene tips
...
def test_payload_budget_under_300g():
    ...
    main_mass = _estimated_mass_g(mount.build_carriage_dpette_mount_main_frictionfit())
    cap_mass = _estimated_mass_g(mount.build_carriage_dpette_mount_cap_frictionfit())
...
def test_main_height_within_z_envelope():
    ...
    Cap derived from FRICTIONFIT_UPPER_TO_LOWER_SEPARATION_MM (80.5 mm,
    re-measured for this iteration) plus top plate + lower clamp stack
    (~5 mm) = ~86 mm, with ~14 mm headroom for future minor adjustments.
    The real i3 Mega Z envelope is 210 mm, so this is a runaway-growth
    guard, not a bed-clearance constraint.
    """
    shape = mount.build_carriage_dpette_mount_main_frictionfit()
...
def test_cap_height_matches_top_plate():
    ...
    cap = mount.build_carriage_dpette_mount_cap_frictionfit()
...
def test_cap_bore_short_axis_matches_main_bore():
    """Cap's capsule-slot short axis must equal the main piece's circular bore.

    Otherwise the cap and main piece's D-cavities wouldn't mate at Y=0.
    """
    expected_short_axis = (
        mount.FRICTIONFIT_UPPER_CLAMP_BORE_D_MM + mount.CLAMP_BORE_CLEARANCE_MM
    )
    assert expected_short_axis < mount.UPPER_CAP_W_MM, (
        f"Bore short axis {expected_short_axis:.2f} mm must fit within cap "
        f"width {mount.UPPER_CAP_W_MM} mm"
    )
...
def test_lbracket_returns_non_empty_shape():
    shape = mount.build_carriage_dpette_mount_main_lbracket_frictionfit()
    assert isinstance(shape, (build123d.Solid, build123d.Compound))
    assert shape.volume > 0
...
def test_lbracket_volume_exceeds_main():
    ...
    main_vol = mount.build_carriage_dpette_mount_main_frictionfit().volume
    lbracket_vol = mount.build_carriage_dpette_mount_main_lbracket_frictionfit().volume
    assert lbracket_vol > main_vol, (
        f"Scheme b ({lbracket_vol:.0f} mm3) must exceed scheme a ({main_vol:.0f} mm3)"
    )


def test_lbracket_payload_budget_under_300g():
    """Scheme b + cap + pipette + tips must fit under the AI3M payload cap."""
    main_mass = _estimated_mass_g(
        mount.build_carriage_dpette_mount_main_lbracket_frictionfit()
    )
    cap_mass = _estimated_mass_g(mount.build_carriage_dpette_mount_cap_frictionfit())
    total = main_mass + cap_mass + PIPETTE_MASS_G + TIPS_MASS_G
    assert total < AI3M_PAYLOAD_CAP_G, (
