...
Ported from so101-biolab-automation (originally Antonio Lamb's CadQuery
source); geometry preserved verbatim.

Usage:
    uv run --extra cad python tools/cad/dpette/dpette_tip_release.py
"""
...
def _base_with_holes():
    """Base plate minus waste slot and 4x M4 through-holes."""
    base = Box(BASE_X, BASE_Y, BASE_Z) - Box(WASTE_SLOT_W, WASTE_SLOT_D, BASE_Z + 2)
    for sx in (-1, 1):
        for sy in (-1, 1):
...
def _arm_and_finger():
    """Vertical arm flush with back of base + horizontal contact finger."""
...
def build_tip_release():
    ...
    return _base_with_holes() + _arm_and_finger()
