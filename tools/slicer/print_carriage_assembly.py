"""Generate and slice the 4-piece carriage mount assembly (friction-fit TESTING).

Pieces: top plate, 2x post, lower clamp — from the friction-fit testing
design in `tools/cad/i3/carriage_dpette_mount_frictionfit.py`, not the
production bolted design in `carriage_dpette_mount.py`.
...
The cap is a separate, much smaller piece — sliced individually, not part
of this batch.
...
OUT_STL_DIR = REPO / "hardware" / "stl" / "i3" / "assembly_frictionfit"
OUT_GCODE = REPO / "hardware" / "gcode" / "carriage_assembly_frictionfit.gcode"

sys.path.insert(0, str(CAD_DIR))


def _post_height_mm() -> float:
    """Mirror the post-length math from build_carriage_dpette_mount_main_frictionfit."""
    from i3.carriage_dpette_mount_frictionfit import (  # type: ignore[import-not-found]
        FRICTIONFIT_UPPER_TO_LOWER_SEPARATION_MM,
...
def export_arranged_stl(pieces: dict[str, object]) -> Path:
    """Arrange all four pieces on the Prusa MK3S+ bed (220x220) and write one STL.
    ...
    and the piece's lowest Z point sits at Z=0 (flat on the bed). Naive Pos(x, y, 0)
    ...
    # Target bed centers (220x220 origin at corner; chosen so pieces
    # don't overlap).
    layout = {
        "top_plate": (110.0, 50.0),  # 35 x 85 -> X[92.5, 127.5], Y[7.5, 92.5]
        "lower_clamp": (
            110.0,
            120.0,
        ),  # 83.5 x 36.5 -> X[68.25, 151.75], Y[101.75, 138.25]
        "post_a": (60.0, 165.0),  # 75 x 12 -> X[22.5, 97.5], Y[159, 171]
        "post_b": (160.0, 165.0),  # 75 x 12 -> X[122.5, 197.5], Y[159, 171]
    }
...
def slice_bed(bed_stl: Path) -> int:
    ...
    Settings are passed as CLI flags rather than loaded from
    `tools/slicer/profiles/*.ini` — an intentional exception to
    `.claude/rules/slicer-profile-source-of-truth.md`. PrusaSlicer 2.9.4's
    `--load` silently drops most profile keys (see docs/prusa/README.md
    Quirk 1); this script predates that workaround and was validated
    against real dry-fit prints with these exact inline flags, most
    importantly the bed-heat start_gcode block (without it, PLA won't
    stick). Left as-is rather than switched to `--load` and re-validated
    against hardware that isn't available in this environment.
    """
    ...
    cmd: list[str] = [
        ...
        "215",  # +5C on first layer for adhesion
        "--bed-temperature",
        "60",
        "--first-layer-bed-temperature",
        "65",  # +5C on first layer for adhesion
        # Start/end blocks — ensures bed heats before printing
        "--start-gcode",
        start_gcode,
        "--end-gcode",
        end_gcode,
        # Bed shape (220x220 MK3S+; MK4 250x210 also accepts this)
        "--bed-shape",
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate, slice, and optionally upload the 4-piece "
        "friction-fit testing carriage mount."
    )
    parser.add_argument(
        "--upload",
        action="store_true",
